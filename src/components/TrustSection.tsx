...
const TrustSection = () => {
  return (
    <section className="relative w-full overflow-hidden bg-background">
      <div className="absolute inset-0 w-full h-full bg-[radial-gradient(ellipse_80%_50%_at_50%_0%,hsl(var(--accent)/0.08),transparent)]" />
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col items-center justify-center overflow-hidden rounded-lg py-20 md:py-28">
              ...
              >
                <img
                  ...
                  className={`object-contain grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300 ${
                    logo.name === 'Colliers' ? 'h-20 md:h-28' :
                    logo.name === 'JLL' ? 'h-10 md:h-14' :
                    logo.name === 'CBRE' ? 'h-8 md:h-12' :
                    'h-14 md:h-20'
                  }`}
                />
              </div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 h-px bg-gradient-to-r from-transparent via-border to-transparent" />
      </div>
    </section>
  );
};

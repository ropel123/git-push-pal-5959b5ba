...
set steps = '[
  ...
  {"action":"act","instruction":"Clique sur le bouton de téléchargement du DCE (Télécharger le DCE, Retirer le dossier, Retrait anonyme, Accéder au dossier de consultation, Consulter le dossier, Dossier de consultation) — surtout PAS un onglet de menu de navigation"},
  ...
  {"action":"act","instruction":"Bouton SUBMIT final du formulaire de retrait DCE : input[type=submit] ou button[type=submit] avec texte Valider / Télécharger / Confirmer / Envoyer / Soumettre / Télécharger tout le DCE. PAS un bouton Annuler, PAS un onglet de menu, PAS un lien de navigation"},
  {"action":"wait_download","timeout_ms":60000}
]'::jsonb,
version = version + 1,
updated_at = now()
where platform in ('atexo_achatpublic','atexo_localtrust');